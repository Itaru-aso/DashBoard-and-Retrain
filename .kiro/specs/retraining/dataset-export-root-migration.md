...
23. **エッジPC配信ファイル名も同様にタプル化する**: `deployment_service.py`の`_remote_name(color_no, mode)`（現状`{color_no}_{mode}_model.onnx`）を`{color_no}_{size}_{chain}_{tape}_{mode}_model.onnx`に変更する。`job`（`RetrainingJob`/`DeployedModel`）が既にsize/chain/tapeを保持しているため、新規のDB問い合わせは不要。配信メカニズム・配信先パス（`remote_dir`等）自体は変更しない。
    - **注意（要確認・未解決）**: エッジPC側の検査アプリケーション（本リポジトリ外）が現状のファイル名`{color_no}_{mode}_model.onnx`を前提に読み込んでいる可能性がある。ファイル名変更がエッジ側で受理されるか、量産昇格フローに影響しないかは本タスクでは未確認。決定21〜22（ローカル保存のタプル化）は本リポジトリ内で完結し安全だが、決定23（配信ファイル名の変更）は外部システムとの契約に触れるため、**エッジ側の受理可否を確認してから実装すること**。
...
- **エッジPC側の検査アプリケーション（リポジトリ外）が、配信ファイル名のタプル化（決定23）を受理できるか未確認**。量産昇格フロー・エッジ側の読み込みロジックへの影響を確認するまで、決定23の実装は保留すべき。決定21・22（ローカル保存のタプル化）はリポジトリ内で完結するため、この確認を待たずに先行実装可能。
