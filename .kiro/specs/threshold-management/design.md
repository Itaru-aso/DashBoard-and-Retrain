...
- モックアップは「3指標カードに値をinline編集→まとめて保存」という単純な構造だが、本 spec は
  スコープ（global/per_color）・有効開始日時による履歴管理（supersedeモデル・R2/R3）を持つため、
  モックアップの構造には簡略化しない。**登録フォーム・一覧・無効化の既存機能はすべて残し、
  見た目（パネル化・ダーク配色・JetBrains Mono表示）のみモックアップに合わせる**（brainstormingで確認済み）。
