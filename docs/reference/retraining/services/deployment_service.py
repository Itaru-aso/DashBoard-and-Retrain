"""ONNX モデルの配信と現行配信モデル記録（ver2）。

配置先: backend/src/services/deployment_service.py

責務（学習と分離。`training_service` から COMPLETED 時に呼ぶ＝v1 自動／将来は手動 API から）:
- COMPLETED ジョブの ONNX（monochro/color）を、有効なエッジPC（`エッジPC管理`）へ **FTP 配信**する。
  FTP は学習側 `ftp_common` に依存せず **ver2 自前の ftplib** 送信（境界を分離）。配信先ポートは `model_port`。
- 検査PC 互換のため、リモートのファイル名は **`{color_no}_{mode}_model.onnx`**（色番ベース）を FTP ルート直下に置く。
- `deployed_model`（**フルタプル単位**＝案A）に現行配信モデルを upsert。全台成功=SUCCESS／一部失敗=PARTIAL／全失敗=FAILED。
  FTP 失敗でもジョブ成功は覆さない（再配信可能な記録として残す）。

依存（注入）:
- `session_factory`: ver2 同期 Session。
- `edge_pc_repo_factory`: Session を受け、`find_enabled()` で有効エッジPC（name/host/username/password/model_port）を返す repo。
- `ftp_sender`: テスト用に差し替え可能な送信関数（既定は ftplib 実装）。

注: import パスはプロジェクトに合わせて調整する。ftplib はブロッキングのため、非同期文脈からは `asyncio.to_thread` で呼ぶ。
"""
...
def _default_ftp_sender(
    ...
    local_path: str, remote_dir: str, remote_name: str, timeout: int,
) -> None:
...
class DeploymentService:
    ...
    def deploy_job(self, job_id: int) -> dict:
        ...
        Returns: 配信結果サマリ（status・per-host 詳細）。
        """
        with self._session_scope() as db:
            ...
            for pc in edge_pcs:
                ...
                for mode, local_path in artifacts:
                    ...
                    try:
                        self._send(
                            host=pc.host, port=pc.model_port,
                            username=pc.username, password=pc.password,
                            local_path=local_path, remote_dir=self._cfg.remote_dir,
                            remote_name=remote_name, timeout=self._cfg.ftp_timeout,
                        )
            ...
            repo.upsert_deployed(
                color_no=job.color_no, size=job.size, chain=job.chain, tape=job.tape,
                job_id=job.id,
                onnx_monochro_path=job.onnx_monochro_path,
                onnx_color_path=job.onnx_color_path,
                deploy_status=status, deploy_detail=detail,
            )
            return {"job_id": job_id, "status": status, "detail": detail,
