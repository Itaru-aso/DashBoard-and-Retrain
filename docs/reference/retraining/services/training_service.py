"""モデル再学習のオーケストレーション（ver2）。

配置先: backend/src/services/training_service.py

責務（学習アルゴリズムは扱わない・`training/pipline.py` を subprocess で起動するだけ）:
...
- 起動: `python pipline.py common.target_color=<color_no> common.pipeline_mode=train
        ...
        を `training/` を CWD に。画像は別機能が `1_download` に事前配置（収集スコープ外）。配信は deployment_service。
- 進捗: subprocess の標準出力を**1行ずつ素通し**で WebSocket 購読者へ配信（揮発）。
- 成功判定: **終了コードに依存しない**。両 mode の ONNX 生成有無＋標準出力の `パイプライン完了` マーカーで判定。
- キャンセル: QUEUED はキューから除外。RUNNING は**プロセスグループごと kill**（spawn 子も含めて停止）。
- 状態は DB を正として都度永続（`RetrainingRepository`）。

注: import パス・設定取得はプロジェクトに合わせて調整する。DB は同期 Session を `asyncio.to_thread` 経由で扱う。
"""
...
class TrainingConfig:
    ...
    model_dir: str                    # ONNX 出力ルート（pipline の common.model_dir。例 training_dir/6_model）
    ...
    def build_command(self, color_no: str) -> list[str]:
        # 色番は文字列。pipline 側 _safe_cli_overrides がクォートして8進数誤変換を防ぐ。
        return [
            self.python_executable, self.entry,
...
class _ProgressHub:
    ...
    def subscribe(self, job_id: int) -> asyncio.Queue[str | None]:
        ...
        # 直近ログを先に流す（後追い接続者向け）
        for line in self._recent.get(job_id, ()):  # noqa: B007
            q.put_nowait(line)
...
class TrainingService:
    ...
    def __init__(
        ...
        on_completed: Callable[[int], "asyncio.Future | None"] | None = None,
    ) -> None:
        """
        Args:
            session_factory: ver2 同期 Session を返すファクトリ（with でクローズ可能なもの）。
            config: 学習側起動の設定。
            on_completed: ジョブ COMPLETED 時に呼ぶフック（deployment_service の自動配信を注入。任意・非同期可）。
        """
    ...
    async def cancel(self, job_id: int) -> bool:
        ...
        Returns: 受理したら True（既に終端なら False）。
        """
    ...
    async def _run_job(self, job_id: int) -> None:
        job = await asyncio.to_thread(self._db_get_tuple, job_id)
        if job is None:
            return
        color_no, size, chain, tape = job
