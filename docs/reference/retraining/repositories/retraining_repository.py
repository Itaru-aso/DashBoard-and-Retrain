"""再学習ジョブ・現行配信モデルのリポジトリ（ver2 DB）。

配置先: backend/src/repositories/retraining_repository.py

責務:
- `retraining_job` の作成・状態遷移（DB を正として永続）・履歴取得。
- `deployed_model`（色フルタプル単位の現行配信モデル）の upsert・取得。
状態遷移の判断ロジック（FIFO・同時1本など）は `training_service` 側が持ち、
本リポジトリは DB 操作に徹する。

注: import パスはプロジェクトのレイアウトに合わせて調整する（例: `from src.models...`）。
"""
...
from models.deployed_model import DeployedModel, DeployStatus
from models.retraining_job import JobStatus, RetrainingJob
...
class RetrainingRepository:
    ...
    def create_job(
        ...
        """ジョブを QUEUED で作成する。"""
        job = RetrainingJob(
            color_no=color_no, size=size, chain=chain, tape=tape or "",
            status=JobStatus.QUEUED.value, queued_at=_now(), created_by=created_by,
        )
        self.db.add(job)
        self.db.flush()  # id 採番
        return job
    ...
    def get(self, job_id: int) -> RetrainingJob | None:
        return self.db.get(RetrainingJob, job_id)
    ...
    def list_active(self) -> list[RetrainingJob]:
        """未終了（QUEUED / RUNNING）のジョブを古い順に返す（起動時の復旧・FIFO 再構築用）。"""
        stmt = (
    ...
    def mark_completed(
        self, job_id: int, onnx_monochro_path: str, onnx_color_path: str,
    ) -> None:
        job = self._require(job_id)
    ...
    def _require(self, job_id: int) -> RetrainingJob:
        job = self.db.get(RetrainingJob, job_id)
        if job is None:
    ...
    def upsert_deployed(
        self, color_no: str, size: str, chain: str, tape: str,
        job_id: int, onnx_monochro_path: str | None, onnx_color_path: str | None,
        ...
        if rec is None:
            rec = DeployedModel(
                ...
                deploy_status=deploy_status, deploy_detail=detail_json,
                deployed_at=_now(),
            )
            self.db.add(rec)
        else:
