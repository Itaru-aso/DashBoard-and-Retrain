...
**全 spec 実装済み**（spec 駆動開発 / cc-sdd）。`backend/`（FastAPI）・`frontend/`（React）とも各機能を TDD で実装し、検証ゲート（backend: black/flake8/mypy/pytest cov≥80、frontend: tsc/eslint/vitest）を通過している。
...
各機能の要件・設計・タスクは [`.kiro/specs/<feature>/`](.kiro/specs/) の `requirements.md` / `design.md` / `tasks.md`（進捗はチェックボックスが台帳）で管理。

> **起動手順は [`docs/manual/起動手順マニュアル.md`](docs/manual/起動手順マニュアル.md) を参照**（Docker / ローカル開発の両方式・環境変数・トラブルシュート）。

## 始め方（開発者向け）

> 詳細な起動手順（Docker / ローカル・環境変数・トラブルシュート）は [`docs/manual/起動手順マニュアル.md`](docs/manual/起動手順マニュアル.md)。規約・前提は [`CLAUDE.md`](CLAUDE.md) を参照。
...
| [`docs/manual/起動手順マニュアル.md`](docs/manual/起動手順マニュアル.md) | アプリ起動手順（Docker / ローカル・環境変数・トラブルシュート） |
