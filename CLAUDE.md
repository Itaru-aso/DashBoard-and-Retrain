...
- **monochro（分母）** ＝ `camera_model` がモノクロ機種コード（`camera1_image`＝開発用ダミー値／`CA-HL04MX`＝実機コード）
  ／ **AI 判定** ＝ `judgment_result`（0:OK / 1:NG）
...
| `@docs/reference/db-app-mapping.md` | ver2 DB 7テーブル↔Repository/Service/API/Job・外部config/env対応、既知のギャップ |
